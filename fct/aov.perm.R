aov.perm <- function(x, y, B = 1000, balanced = TRUE) {
  ...
  for (i in 1:(C - 1)) {
    for (j in (i + 1):C) {
      name[cont] <-
        paste(unique(y[unclass(y) == i]), unique(y[unclass(y) == j]), sep = "-")
      cont = cont + 1
    }
    }
  sig <- function(p) {
  ...
  for (i in 1:(C - 1)) {
    for (j in (i + 1):C) {
      T[1,cont] <- N[i] * N[j] *
        (mean(x[unclass(y) == i]) - mean(x[unclass(y) == j]))^2
  ...
  for (bb in 2:(B + 1)) {
    if (balanced == TRUE) {
      ...
      for (i in 1:(C - 1)) {
        for (j in (i + 1):C) {
          T[bb, cont] <- N[i] * N[j] *
            (mean(X.perm[c(1:n), cont]) - mean(X.perm[-c(1:n), cont]))^2
          ##2nd sol
          cont = cont + 1
        }
        }
    ...
    if (balanced==FALSE) {
      ...
      for (i in 1:(C - 1)) {
        for (j in (i + 1):C) {
          T[bb, cont] <-
            N[i] * N[j] * (mean(x.perm[unclass(y) == i]) - mean(x.perm[unclass(y) == j])) ^ 2
          cont = cont + 1
        }
      }
      }##End balanced = FALSE
  ...
  T1 <- apply(T, 1, sum)
  P.part[, 1] <- apply(T, 2, function(x) {
    sum(x[-1] >= x[1]) / B
  })
  P.part <- data.frame(Diff = Diff,
                       p = P.part,
                       sig. = sig(P.part))
